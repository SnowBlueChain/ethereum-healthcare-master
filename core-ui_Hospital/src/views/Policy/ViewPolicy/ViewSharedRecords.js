...
import {userdetails, storage, organization, permissions, policy} from "../../../contract_abi";
...
export class ViewSharedRecords extends Component {
    constructor(props) {
      ...
      instantiateContract() {
          ...
          console.log("SessionL", sessionStorage.getItem('policyAddress'))
            const policyContractAddress = sessionStorage.getItem('policyAddress')
                ...
                {from: accounts[0]}, (error, details) => {
                  if(!error) {
                    if(details[2] === sessionStorage.getItem("orgId")) {
                        this.policyContract.methods.getRecords().call(
